...
export default function PollsPage() {
  ...
  const fetchPolls = async () => {
    ...
    try {
      ...
      // Optionally fetch based on user location if available
      // Let's assume the backend handles optional query parameters
      if (user?.location) {
      ...
      // Assume the response might wrap it if it's `{ success: true, polls: [] }` or return directly `[]`
      setPolls(Array.isArray(data) ? data : (data.polls || []));
    } catch (err: any) {
      setError(err.message || "Failed to load polls. Is the backend running?");
    } finally {
      setLoading(false);
    }
  };
  ...
  const canCreate = user?.role === "official" || user?.role === "admin" || user?.role === "Official" || user?.role === "Admin";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-lg font-medium">No polls found</p>
            <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">
              {search ? "Try adjusting your search" : "Check back later for new polls."}
            </p>
          </div>
        ) : (
          <>
